...
string Priority_process() {
	return to_string(GetPriorityClass(GetCurrentProcess()));
}
string Id_n_process_descriptor() {
	DWORD x = GetCurrentProcessId();
	HANDLE y = OpenProcess(PROCESS_ALL_ACCESS, true, x);
	...
	string s_y = to_string(z);
	result += to_string(s_y.length()) + s_y;
...
void function(SOCKET ClientConn, short BUFF_SIZE, int number_client) {
	...
	string menu = "1. Вывод приоритета серверного процесса\n2. Вывод идентификатора и дескриптора серверного процесса\n0. Выход\nВведите число: ";
	...
	while (true) {
		...
		if (servBuff[0] == '1') {
			string output = "1 сервер: Клиент с номером " + to_string(number_client) + " подал запрос на приоритет процесса.";
			WriteInLog("\n" + GetSystemForLog() + output + "\n");
			result = Priority_process();
		}
		if (servBuff[0] == '2') {
			string output = "1 сервер: Клиент с номером " + to_string(number_client) + " подал запрос ИД процесса и дескриптор.";
			WriteInLog("\n" + GetSystemForLog() + output + "\n");
			result = "h" + Id_n_process_descriptor(); 
		}
	...
	string output = "1 сервер: Клиент с номером " + to_string(number_client) + " был отключен. ";
	WriteInLog("\n" + GetSystemForLog() + output + "\n");
}
...
int main(void) {
	...
	HANDLE ServerCanStartWork = CreateEvent(NULL, FALSE, FALSE, L"WorkPipe1");
	...
	WriteInLog("\n" + GetSystemForLog() + "  Запущен первый сервер." + "\n");
	...
	WriteInLog("\n" + GetSystemForLog() + "1 сервер: Инициализация WinSock прошла успешно." + "\n");
	...
	WriteInLog("\n" + GetSystemForLog() + "1 сервер: Инициализация серверного сокета прошла успешно." + "\n");
	...
	WriteInLog("\n" + GetSystemForLog() + "1 сервер: Привязка сокета к информации о сервере прошла успешно." + "\n");
	...
	while (true) {
		...
		if (!(ClientConn == INVALID_SOCKET)) {
			...
			WriteInLog("\n" + GetSystemForLog() + "1 сервер: Соединение с клиентом успешно установлено." + "\n");
			char clientIP[22];
